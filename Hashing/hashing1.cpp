...
public:
    HashTable(CollisionMethod method, int hashchoice)
        : method(method), hashchoice(hashchoice)
    {
    ...
    void insert(K key, V value)
    {
        ...
        {
            // Collision Check
            if (table[h].isOccupied && !table[h].isDeleted && table[h].key != key)
            {
                totalCollisions++;
            }
            ...
            for (int i = 0; i < tableSize; i++)
            {
                ...
                if (insertIdx == -1 && (!table[idx].isOccupied || table[idx].isDeleted))
                {
                    insertIdx = idx;
                }
            ...
            if (insertIdx != -1)
            {
                table[insertIdx] = {key, value, true, false};
            }
            else
            {
                return; // full
